...
shortcut_key_values! { "listing",
    ...
    pub struct ListingShortcuts {
        ...
        search |> "Search within list of e-mails." |> ShortcutKeys::double(Key::Char('/'), Key::F(3)),
        refresh |> "Manually request a mailbox refresh." |> ShortcutKeys::double(Key::F(5), Key::Ctrl('r')),
        ...
        focus_right |> "Open the entry under the cursor; the focus stays on the mail list." |> ShortcutKeys::double(Key::Right, Key::Char('l')),
...
shortcut_key_values! { "general",
    pub struct GeneralShortcuts {
        ...
        enter_command_mode |> "Enter COMMAND mode." |> ShortcutKeys::double(Key::Char(':'), Key::Alt('x')),
