...
impl ListingTrait for PlainListing {
    ...
    fn highlight_line(&mut self, grid: &mut CellBuffer, area: Area, idx: usize, context: &Context) {
        ...
        let selected = self.rows.selection.get(&i).copied().unwrap_or(false);
            ...
            for &(idx, highlight) in &[(old_cursor_pos.2, false), (self.new_cursor_pos.2, true)] {
                ...
                if highlight {
                    let selected = self
                        ...
                        .map(|h| self.rows.selection.get(&h).copied().unwrap_or(false))
                        .unwrap_or(false);
    ...
    fn draw_relative_numbers(
        ...
        for i in 0..area.height() {
            ...
            let row_attr = if let Some(env_hash) = self.get_env_under_cursor(top_idx + i) {
                ...
                let selected = self.rows.selection.get(&env_hash).copied().unwrap_or(false);
                let row_attr = row_attr!(
                    self.color_cache,
                    unseen: unseen,
                    highlighted: highlighted,
...
impl Component for PlainListing {
    ...
    fn process_event(&mut self, event: &mut UIEvent, context: &mut Context) -> bool {
        ...
        // bindings, so skip rebuilding (and re-hashing) this map — which
        // also clones the focused component's and the mail view's shortcut
        // sections — for every other event: backend syncs can deliver
