...
- Three-segment status bar (plan `statusbar-gauge-spinner`): the bottom row is now split via `Layout::horizontal` into a left status segment, a centre `LineGauge` driven by the focused mailbox's `MailboxStatus::Parsing(done, total)`, and a right hints segment that lists `q:quit ?:toggle_help F5:refresh` (truncated with `…` / `...` when the row is narrow). The backend chip `[maildir]` / `[imap ✓]` / `[imap ✘]` and the focused mailbox label (`📩 INBOX:42`) live at the right edge of the left segment and are derived at draw-time from `context.accounts` plus a new `StatusBar.focus: Option<(AccountHash, MailboxHash)>`. ASCII terminals use `+`/`x` and drop the envelope glyph. The centre gauge uses `filled_symbol # / .` and `filled_style` is `status.bar` inverted (same palette fork the Insert mode indicator uses) so no new theme key is introduced.

- Status-bar data channel: new `StatusEvent::FocusMailbox(AccountHash, MailboxHash)` lets the active listing/tree component report which mailbox the user is focused on, plus a default `Component::status_watch()` trait method (Listing overrides it to return its cursor's `(AccountHash, MailboxHash)`, Tabbed forwards the active child's). `StatusBar` consumes `FocusMailbox` to track focus; it also gained non-consuming `MailboxUpdate((acc, mb))` and `AccountStatusChange(acc, _)` arms that mark the bar dirty only when the event's `AccountHash` matches the focused account's, so off-focus refreshes don't trigger redraws. All eight `UpdateStatus` send sites inside `Listing` (and the two inside `Tabbed`) were refactored through a single helper that emits both the legacy string and the structured focus event.
...
- Command palette (VSCode Ctrl+P style, plan `command-palette`): pressing `:` / `M-x` now opens a centered floating panel (corners at 10%/90% of the screen, 80% × 80%) over the current UI instead of the old second bottom command-line row. A single-line input (`ratatui-textarea`, backspace/arrows/Home/End supported) sits at the panel top; below it, a fuzzy-matched list (`nucleo-matcher`, smart case, score-descending) shows every command from `COMMAND_COMPLETION` plus history entries, with matched characters highlighted bold+underline. An empty query lists `Frequent` (commands used more than once, most-used first, top 10) and `History` (newest-first dedup, top 10) sections — or all commands when there is no history. Keys: `Tab` completes the input to the selected entry, `Enter` executes it (or the raw input) and closes, `Up`/`Down` (`Ctrl-P`/`Ctrl-N`) move the selection, `Esc` closes without executing. Execution is unchanged: the palette only queues `UIEvent::Command`, so parsing, `needs_confirmation` dialogs (e.g. quit) and history logging (`cmd_history`) behave exactly as before; the listing's `/` and F3 search prefill still lands in the input. The old ex-buffer/`AutoComplete` plumbing was removed from `StatusBar` together with the dead token-completion machinery (`Token`/`TokenStream`/`command_completion_suggestions`); the command-mode bottom strip is gone, leaving the single status row (the mode indicator still reads COMMAND). New dependencies: `nucleo-matcher` 0.3 and `ratatui-textarea` 0.9 (no default features — no backend pulled in; `ratatui-core`/`ratatui-widgets` versions unify with the existing ratatui 0.30 facade). The `TextArea` lives behind a `Mutex` because its render caches are `!Sync` interior-mutable cells.

- `open_in_new_tab` thread-view shortcut (default `Enter`): pressing it while the mail content pane holds the keyboard — every layout's mail view (the two-pane mail layout's right pane, the thread layout's mail pane) — opens the mail being viewed in a new tab, exactly the `open-in-tab` command's action (the ThreadView re-dispatches the keypress as `ListingAction::OpenInNewTab`). An open mail view dialog (URL launch, List-Unsubscribe confirmation, …) keeps Enter for itself, and the shortcut stays inert while the thread list or the mail listing grid holds the keyboard. Configurable via `[shortcuts.thread-view]`.
...
- Layout stability across background account refreshes (163/IMAP reconnect loop): a `UIEvent::AccountStatusChange` for the current account (the watcher reconnecting — "Establishing TLS connection." / "Attempting authentication." — or the post-reconnect mailbox list reconcile, "Refreshed mailboxes.") used to run the full `Listing::change_account` teardown, whose unconditional `close_view` collapsed whatever layout was on screen back to layout1. The reconcile path now keeps the open view: `change_account` gained a `keep_view` flag that skips the `close_view` teardown, skips `set_coordinates` when the mailbox is unchanged (it resets the grid's entry focus and filter state, which would strand the open view on an unfocused grid — the view's draw gate is `component.unfocused()`), and `set_index_style` no longer closes the view on a no-op style switch (same style already rendered). A stale guard still closes the view if the reconcile lands the grid on a different mailbox or nothing is left under the cursor, and a `View` keyboard focus (layout4) survives while its view does. The view/keyboard highlight flags (`set_grid_focused` / `set_grid_has_keyboard`) now respect a surviving `View` focus when the refresh re-kicks `OpenEntryUnderCursor`. Regression tests: `account_status_change_keeps_open_view` (layout2, incl. a draw check that the mail pane actually renders), `account_status_change_keeps_view_focus` (layout4), `account_status_change_closes_stale_view_on_moved_mailbox` (stale close).

- IMAP offline-cache walk pages by rows, not UID windows (163/Coremail spinner storm): the `CacheFirst`/`FromCache` fetch stages advanced the cache walk by `max_uid -= batch_size` in UID space, so on long-lived servers with sparse UID spaces (`UIDVALIDITY = 1`, `uidnext` in the 10^8-10^9 range, a handful of real messages) one fetch iterated thousands of near-empty cache windows — each an instant sqlite query plus a `MailboxUpdate` payload — keeping the status-bar mailbox spinner on permanently (log evidence: ~50 000 `MailboxUpdate`s per minute across four mailboxes for the whole session, ~600 instant `fetch-mailbox-continued` chunks per fetch). `ImapCache::envelopes` now returns the newest `batch_size` rows `ORDER BY uid DESC LIMIT ?` together with the page's lowest UID (quarantine placeholders ride the same page window), and both stages advance the next page to `lowest − 1`, ending when the page is empty or hits UID 1: the walk takes `O(cached_rows / batch_size)` queries however sparse the UID space, so an initial fetch finishes in seconds and later syncs stay incremental.

- Exit-to-layout1 lands the keyboard on the mailbox list: pressing the exit key (`i`/`q`/`Esc` per `exit_entry` and the general quit bindings) from any layout that closes the open view back to layout1 (layout2's grid or mail view, layout3's grid) used to leave the keyboard on the mail list grid; it now lands on the mailbox sidebar whenever the sidebar is visible — the same landing `focus_left` already uses. A hidden sidebar (`menu_visibility = false`) keeps the keyboard on the grid, and intermediate quits (layout4 → layout3) are unchanged. The `conversations_entry_close_no_residue` golden was re-recorded (the sidebar ring renders focused, the grid ring dimmed). Tests: `quit_key_exits_open_mail_view` (now asserts the `Menu` landing), `layout3_left_goes_to_mailbox_list_and_l4_quit` (quit from layout3 lands on the mailbox list with the sidebar visible).
...
- `MailboxStatus::Parsing` total preservation across incremental fetch batches (plan `statusbar-gauge-spinner`): the incremental `Fetch` handler in `Account::process_event` previously zeroed the running total on every batch (`Parsing(prev_len + len, 0)`), which made any `LineGauge` rendering on top of `MailboxStatus` un-recoverable after the first batch. Both fields are now read out of the previous entry and written back unchanged alongside the accumulated `done` count, with a comment explaining why the total is load-bearing.
